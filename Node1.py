...
class Node:
    ...
    def start(self, start_time, file_id, des_node, nodes):
        ...
        for item in range(15):
            self.env.process(self.generate_data(file_id,item,start_time,des_name))
    ...
    def generate_data(self,file_id,item,start_time,des_name):
        yield self.env.timeout(item*0.02)
        packet = Packet(file_id=file_id, packet_id=item, start_time=start_time, source_now=self.name,
                        destination_now=des_name,
                        source_global=self.name, destination_global=des_name, data=item*20)
        self.packets[file_id].append(packet)
        print('Generate packet',packet.packet_id,'at',round(self.env.now, 2))
    ...
    def source_send(self, file_id, des_node, nodes):
        ...
        while self.success_set[file_id] == 'Fail':
            ...
            yield self.env.process(self.timer_exact(file_id))


    def timer_exact(self,file_id):
    ...
    def source_receive(self, packet):
        yield self.env.timeout(0)
        ack = packet.packet_id
        file_id = packet.file_id
        print('Node', self.name, 'got ACK ', ack,'at',round(self.env.now, 2))
    ...
    def destination_receive(self, packet, nodes):
        ...
        print('Node', self.name, 'receives packet —— file_id: ', packet.file_id, ', packet_id: ', seq_num,'at',round(self.env.now, 2))
        ...
        if seq_num is not None:
            if seq_num == self.expected_num_set[packet.file_id]:
                print('Node', self.name, 'Got expected packet, Sending ACK', self.expected_num_set[packet.file_id],'at',round(self.env.now, 2))
                ...
                packet_ack = Packet(file_id=packet.file_id, packet_id=self.expected_num_set[packet.file_id],
                                    ...
                                    source_global=packet.source_global,
                                    destination_global=packet.destination_global,
                                    data=None)
                delay = 0.01
                ...
                print('Node', self.name, 'Not Got expected packet, Sending ACK',self.expected_num_set[packet.file_id] - 1,'at',round(self.env.now, 2))
                packet_ack = Packet(file_id=packet.file_id,
                                    packet_id=self.expected_num_set[packet.file_id] - 1,
                                    start_time=packet.start_time,
                                    source_now=self.name, destination_now=packet.source_global,
                                    source_global=packet.source_global,
                                    destination_global=packet.destination_global,
    ...
    def send(self, packet, delay, des_node, nodes):
        ...
        print('Node', self.name, 'sending packet —— file_id: ', packet.file_id, ', packet_id: ', packet.packet_id,'at',round(self.env.now, 2))
    ...
    def recv(self, packet, nodes):
        ...
        if self.name == packet.source_global:
            self.env.process(self.source_receive(packet))
